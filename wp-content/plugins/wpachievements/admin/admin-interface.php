...
function wpach_of_reset_options($options,$page = ''){
...
function wpach_siteoptions_options_page(){
    $options =  get_option('wpach_of_template');      
    $themename =  get_option('wpach_of_themename');
?>
...
<div class="wrap" id="truethemes_container">
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      <div class="logo">
        <img src="<?php echo plugins_url('wpachievements/img/logo.png'); ?>" alt="WPAchievements Logo" width="45" style="float:left;margin:2px 7px 0 -2px;" />
        <h2><?php echo __('WPAchievements Settings', WPACHIEVEMENTS_TEXT_DOMAIN); ?></h2>
      </div>
    ...
    // Rev up the Options Machine
        $return = wpach_siteoptions_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
    ...
    <img style="display:none;" src="<?php echo plugins_url('wpachievements/admin/images/wpspin_light.gif') ?>" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
  ...
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    ...
    <input name="reset" type="submit" value="Reset Options" class="submit-button reset-button" onclick="return confirm('CAUTION: Any and all settings will be lost! Click OK to reset.');" />
...
function wpach_of_style_only(){
  wp_enqueue_style('wpachievements-admin-style', plugins_url('wpachievements/admin/admin-style.css'));
  wp_enqueue_style('wpachievements-color-picker', plugins_url('wpachievements/admin/colorpicker.css'));
}
...
function wpach_of_load_only(){

  wp_enqueue_script('wpachievements-jquery-ui-core');
  wp_register_script('jwpachievements-query-input-mask', plugins_url('wpachievements/admin/js/jquery.maskedinput-1.2.2.js'), array( 'jquery' ));
  wp_enqueue_script('wpachievements-jquery-input-mask');
  wp_enqueue_script('wpachievements-color-picker', plugins_url('wpachievements/admin/js/colorpicker.js'), array('jquery'));
  wp_enqueue_script('wpachievements-ajaxupload', plugins_url('wpachievements/admin/js/ajaxupload.js'), array('jquery'));
  
  ?>
    <script>
  jQuery(document).ready(function(){
    jQuery("span.switch").parent().next().css('cssText', 'margin:5px 0 0 50px;');
    jQuery("span.switch").parent().next().find('small').parent().css('cssText', 'margin:0 0 0 50px;')
    jQuery(".cb-enable").click(function(){
          var parent = jQuery(this).parents('.switch');
          jQuery('.cb-disable',parent).removeClass('selected');
          jQuery(this).addClass('selected');
          jQuery('.checkbox',parent).attr('checked', true);
      jQuery('.checkbox',parent).val('true');
      var tabid = '#'+jQuery(parent).parent().attr('id');
      jQuery(tabid + ' textarea').each(function(){
...
<script type="text/javascript" language="javascript">
    ...
    if (typeof AjaxUpload != 'function'){ 
      ...
      //Color Picker
      <?php $options = get_option('wpach_of_template');
      ...
      foreach($options as $option){ 
        ...
        ?>
        <?php if( empty($color) ){ $color = '#444444'; }?>
         ...
         jQuery('#<?php echo $option_id; ?>_picker').ColorPicker({
          ...
          jQuery('#<?php echo $option_id; ?>_picker').next('input').bind('keyup',function(){
            if( jQuery(this).val().indexOf('#') >= 0 ){
              var thiscol = jQuery(this).val();
            } else{
              var thiscol = '#'+jQuery(this).val();
            }
            jQuery('#<?php echo $option_id; ?>_picker').ColorPickerSetColor( thiscol );
            jQuery('#<?php echo $option_id; ?>_picker').children('div').css('backgroundColor', thiscol);
          })
        <?php } } ?>
...
<script type="text/javascript">
      ...
      new AjaxUpload(clickedID, {
          ...
          data: { // Additional data to send
            action: 'wpa_of_ajax_post_action',
            type: 'upload',
            data: clickedID },
  ...
  jQuery("a[title=submit]").click( function(){
    ...
    jQuery(this).parents("form").submit();
      ...
      jQuery('#ofform').submit(function(){
          ...
          if( jQuery('#wpachievements_pcheck').val() != 0 && jQuery('#wpachievements_pcheck').val() < 5 ){
            alert('<?php _e( 'Popup Automatic Checks must be set no lower then 5.', WPACHIEVEMENTS_TEXT_DOMAIN ); ?>');
            return false;
          }
        ...
        //Save everything else
      jQuery('#ofform-reset').submit(function(){
...
add_action('wp_ajax_wpa_of_ajax_post_action', 'wpach_of_ajax_callback');

function wpach_of_ajax_callback(){
  global $wpdb; // this is how you get access to the database
  
  
  $save_type = $_POST['type'];
  
  if($save_type == 'total_reset'){
    ...
    foreach($options as $option_array){
      ...
      if(isset($option_array['id'])){ // Non - Headings...
          ...
          elseif($new_value == '' && ($type == 'checkbox' || $type == 'checkbox2')){ // Checkbox Save
            update_option($id,'false');
          }
          elseif ($new_value == 'true' && ($type == 'checkbox' || $type == 'checkbox2')){ // Checkbox Save
...
function wpach_siteoptions_machine($options){
  ...
  foreach ($options as $value){
    ...
    case "checkbox": 
      ...
      if($checked){
        $labels = '<span class="switch"><label class="cb-enable selected"><span>'.__( 'On', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label><label class="cb-disable"><span>'.__( 'Off', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label>';        
        $input = '<input type="checkbox" class="checkbox" name="'. $value['id'] .'" id="'. $value['id'] .'" value="true" checked="checked" />';
      } else{
        $labels = '<span class="switch"><label class="cb-enable"><span>'.__( 'On', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label><label class="cb-disable selected"><span>'.__( 'Off', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label>';        
        $input = '<input type="checkbox" class="checkbox" name="'. $value['id'] .'" id="'. $value['id'] .'" value="false" />';
      }
    ...
    case "checkbox2": 
      ...
      if($checked){
        $labels = '<span class="switch"><label class="cb-enable selected"><span>'.__( 'Yes', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label><label class="cb-disable"><span>'.__( 'No', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label>';        
        $input = '<input type="checkbox" class="checkbox" name="'. $value['id'] .'" id="'. $value['id'] .'" value="true" checked="checked" />';
      } else{
        $labels = '<span class="switch"><label class="cb-enable"><span>'.__( 'Yes', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label><label class="cb-disable selected"><span>'.__( 'No', WPACHIEVEMENTS_TEXT_DOMAIN ).'</span></label>';        
        $input = '<input type="checkbox" class="checkbox" name="'. $value['id'] .'" id="'. $value['id'] .'" value="false" />';
      }
    ...
    if($value['type'] != "heading"){
      ...
      if($value['type'] == "checkbox2"){
        $output .= '</div><div class="explain"><div class="checkbox2-explain"><strong>'. $explain_value .'</strong></div></div>'."\n";
      } else{
...
function wpach_siteoptions_uploader_function($id,$std,$mod){
