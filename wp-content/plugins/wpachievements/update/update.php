...
class PluginUpdateChecker_1_3_1 {
	...
	public function displayManualCheckResult() {
      ...
      $update_response = get_option('external_updates-'.$_GET['puc_slug']);
			if ( $status == 'no_update' ) {
				$message = 'This plugin is up to date.';
			} else if ( $status == 'update_available' ) {
				$message = 'A new version of WPAchievements is available.';
        if( isset($update_response->update->license->status) ){
          if( $update_response->update->license->status == 'invalid' ){
            $message .= '..&nbsp;<strong><font color="#FF0000">'.$update_response->update->license->error.'</font></strong>';
          }
        }
			} else {
...
class PluginUpdate_1_3 {
	...
	public $download_url;
	public $upgrade_notice;
	private static $fields = array('id', 'slug', 'version', 'homepage', 'download_url', 'upgrade_notice', 'license');
